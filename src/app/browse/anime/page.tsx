...
const GENRES = ['Action', 'Slice of Life', 'Romance', 'Psychological', 'Adventure', 'Comedy'];
...
export default function AnimePage() {
  ...
  const { data, isLoading, error } = useContentByType('ANIME');
    ...
    <>
      ...
      <main className="min-h-screen bg-dark-900">
        <div className="container mx-auto px-4 py-12">
          ...
          {error ? (
            <p className="text-red-400 text-center py-12">Failed to load anime. Please try again.</p>
          ) : (
              ...
              {isLoading
                ...
                ? (
                  <p className="col-span-full text-center text-gray-400 py-12">
                    No anime found{activeGenre ? ` in ${activeGenre}` : ''}.
                  </p>
                )
                : filtered?.map((item) => (
                    <ContentCard
                      ...
                      rating={item.malRating ?? item.imdbRating ?? undefined}
